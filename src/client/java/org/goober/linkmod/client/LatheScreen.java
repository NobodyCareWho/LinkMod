...
import org.goober.linkmod.screenstuff.LatheScreenHandler;
import org.goober.linkmod.recipestuff.LatheRecipe;

@Environment(EnvType.CLIENT)
public class LatheScreen extends HandledScreen<LatheScreenHandler> {
    ...
    private static final Identifier TEXTURE = Identifier.of("lmod", "textures/gui/container/lathe.png");
    ...
    public LatheScreen(LatheScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    protected void drawMouseoverTooltip(DrawContext context, int x, int y) {
        ...
        if (this.canCraft) {
            ...
            CuttingRecipeDisplay.Grouping<LatheRecipe> grouping = ((LatheScreenHandler)this.handler).getAvailableRecipes();
    ...
    private void renderRecipeBackground(DrawContext context, int mouseX, int mouseY, int x, int y, int scrollOffset) {
        for(int i = this.scrollOffset; i < scrollOffset && i < ((LatheScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
            int j = i - this.scrollOffset;
            int k = x + j % 4 * 16;
            int l = j / 4;
            int m = y + l * 18 + 2;
            Identifier identifier;
            if (i == ((LatheScreenHandler)this.handler).getSelectedRecipe()) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.canCraft) {
            ...
            for(int l = this.scrollOffset; l < k; ++l) {
                ...
                if (d >= 0.0 && e >= 0.0 && d < 16.0 && e < 18.0 && ((LatheScreenHandler)this.handler).onButtonClick(this.client.player, l)) {
                    MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_STONECUTTER_SELECT_RECIPE, 1.0F));
                    this.client.interactionManager.clickButton(((LatheScreenHandler)this.handler).syncId, l);
                    return true;
                }
            }
    ...
    private boolean shouldScroll() {
        return this.canCraft && ((LatheScreenHandler)this.handler).getAvailableRecipeCount() > 12;
    }

    protected int getMaxScroll() {
        return (((LatheScreenHandler)this.handler).getAvailableRecipeCount() + 4 - 1) / 4 - 3;
    }

    private void onInventoryChange() {
        this.canCraft = ((LatheScreenHandler)this.handler).canCraft();
        if (!this.canCraft) {
