...
public class AgentPillagerEntityModel extends IllagerEntityModel<IllagerEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		modelPartData.addChild("leftLeg", ModelPartBuilder.create()
				...
				ModelTransform.rotation(2.0F, 12.0F, 0.0F));
	...
	public void setAngles(IllagerEntityRenderState state) {
		...
		float limbSwingProgress = state.limbSwingAnimationProgress;

		if (!state.hasVehicle) {
			this.rightArm.pitch = MathHelper.cos(limbSwingProgress * 0.6662F + (float)Math.PI) * 2.0F * limbSwingAmplitude * 0.5F;
			this.leftArm.pitch = MathHelper.cos(limbSwingProgress * 0.6662F) * 2.0F * limbSwingAmplitude * 0.5F;
		}

		IllagerEntity.State illagerState = state.illagerState;
